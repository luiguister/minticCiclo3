...
def calificar():
    #busqueda = Reserva.query.filter_by(huesped=session['identification']).first()
    busqueda = Reserva.query(Reserva.room).filter(Reserva.huesped==session['identification']).first()
    db.session.commit()
    return render_template('calificar.html', busqueda=busqueda)
